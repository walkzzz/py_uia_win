...
# 先导入driver_factory，然后猴子补丁替换它
try:
    from rf_win.drivers.automation_driver import driver_factory
    # 替换driver_factory为我们的模拟实现
    sys.modules['rf_win.drivers.automation_driver'].driver_factory = MockDriverFactory()
except Exception as e:
    print(f"警告: 无法导入driver_factory，将在库导入后尝试替换: {e}")
